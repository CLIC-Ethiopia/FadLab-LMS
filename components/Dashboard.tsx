...
const CustomTooltip = ({ active, payload, label }: any) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className="glass dark:glass p-4 rounded-xl shadow-2xl border border-white/20 dark:border-white/5 animate-fade-in">
        <p className="font-bold text-slate-800 dark:text-white mb-1">{label}</p>
        <div className="space-y-1 text-xs">
          <p className="flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-indigo-50 shadow-[0_0_8px_rgba(99,102,241,0.6)]"></span>
    ...
    <div className="space-y-8 animate-fade-in pb-12">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="flex flex-wrap gap-3">
             <div className="flex items-center gap-2 px-4 py-2 glass dark:glass border border-indigo-100 dark:border-white/10 rounded-2xl shadow-sm">
             ...
             <div className="flex items-center gap-2 px-4 py-2 glass dark:glass border border-amber-100 dark:border-white/10 rounded-2xl shadow-sm">
                ...
                <div>
                   ...
                   <p className="text-sm font-bold text-amber-700 dark:text-indigo-200 leading-none">{student.points} Points</p>
                </div>
             </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-5">
           <div className="glass dark:glass p-6 rounded-[2.5rem] border border-white/20 dark:border-white/5 shadow-2xl transition-all hover:shadow-indigo-500/10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <div key={i} className="glass dark:glass p-5 rounded-2xl shadow-lg border border-white/20 dark:border-white/5 flex items-center gap-4 transition-all hover:scale-[1.02]">
            <div className={`p-3 rounded-xl bg-${stat.color}-50/50 dark:bg-${stat.color}-900/20 text-${stat.color}-600 dark:text-${stat.color}-400`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="glass dark:glass p-8 rounded-[2.5rem] border border-white/20 dark:border-white/5 lg:col-span-2 shadow-xl transition-colors">
          <div className="flex justify-between items-center mb-8">
             <div>
                <h3 className="text-xl font-bold text-slate-900 dark:text-white uppercase tracking-tight">Active Curriculum</h3>
                <p className="text-sm text-slate-500">Your current path through the industrial framework.</p>
             </div>
             <span className="text-xs font-black px-3 py-1 bg-slate-900/10 dark:bg-white/10 text-slate-500 dark:text-slate-400 rounded-full">
        ...
        <div className="glass dark:glass p-8 rounded-[2.5rem] border border-white/20 dark:border-white/5 shadow-xl transition-colors">
          <h3 className="text-xl font-bold text-slate-900 dark:text-white uppercase tracking-tight mb-6">Innovation Leaderboard</h3>
              ...
              <div key={s.id} className="flex items-center justify-between group">
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black text-sm transition-all group-hover:scale-110
                    ${idx === 0 ? 'bg-amber-100/80 text-amber-700 dark:bg-amber-900/40 dark:text-amber-500 ring-2 ring-amber-500/20' : 
                      'bg-white/40 dark:bg-white/5 text-slate-500 dark:text-slate-400'
                    }`}>
                    {idx + 1}
                  </div>
          ...
          <div className="mt-8 pt-6 border-t border-slate-100 dark:border-white/10">
            <button className="w-full py-3 bg-white/20 dark:bg-white/5 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-[0.2em] rounded-xl hover:bg-indigo-500 hover:text-white transition-all">
      ...
      <div className="glass dark:glass p-8 rounded-[3rem] border border-white/20 dark:border-white/5 shadow-xl transition-colors">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
           <div>
              <h3 className="text-2xl font-black text-slate-900 dark:text-white flex items-center gap-2 uppercase tracking-tight">
                <Target className="w-6 h-6 text-indigo-500" />
                Strategic Study Map
              </h3>
              <p className="text-slate-500">Manage your industrial research milestones.</p>
           </div>
           <div className="h-px bg-slate-100 dark:bg-white/10 flex-grow mx-6 hidden md:block"></div>
        </div>
        
        {enrollments.length === 0 ? (
          <div className="text-center py-20 bg-slate-50/50 dark:bg-white/5 rounded-[2rem] border-2 border-dashed border-slate-200 dark:border-white/10 group">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {enrollments.map((enrollment, index) => {
              ...
              let statusColor = "bg-slate-100/80 text-slate-600 dark:bg-white/5 dark:text-slate-400";
              ...
              if (hasValidStart && hasValidTarget) {
                 if (today < startDateObj) {
                    ...
                    statusColor = "bg-blue-100/80 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300";
                 } else {
                    const daysLeft = Math.ceil((targetDateObj.getTime() - today.getTime()) / (1000 * 60 * 60 * 24));
                    if (daysLeft < 0) {
                        statusText = "SCHEDULE OVERDUE";
                        statusColor = "bg-red-100/80 text-red-700 dark:bg-red-900/30 dark:text-red-300";
                    } else {
                        statusText = `${daysLeft} DAYS TO DEADLINE`;
                        statusColor = daysLeft <= 7 ? "bg-orange-100/80 text-orange-700 dark:bg-orange-900/30" : "bg-green-100/80 text-green-700 dark:bg-green-900/30";
                    }
                 }
              }

              return (
                <div key={index} className="glass dark:glass rounded-[2.5rem] border border-white/20 dark:border-white/5 hover:shadow-2xl hover:shadow-indigo-500/10 hover:-translate-y-1 transition-all duration-300 group flex flex-col h-full relative overflow-hidden">
                  ...
                  <div className="p-6 pb-0 flex items-start gap-4">
                    <div className="relative flex-shrink-0">
                       <img src={course.thumbnail} alt="" className="w-16 h-16 rounded-2xl object-cover shadow-2xl group-hover:scale-105 transition-transform duration-500 border border-white/20" />
                       <div className="absolute -bottom-1 -right-1 p-1 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md rounded-lg shadow-xl border border-white/20 dark:border-white/10">
                         <ShieldCheck className="w-3 h-3 text-blue-500" />
                       </div>
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-start gap-2">
                        <h4 className="font-black text-slate-800 dark:text-white line-clamp-1 group-hover:line-clamp-none transition-all duration-300 uppercase tracking-tighter">
                          {course.title}
                        </h4>
                        <button onClick={(e) => { e.stopPropagation(); if (onEditGoal) onEditGoal(course); }} className="p-1.5 text-slate-300 hover:text-indigo-600 glass dark:glass rounded-lg transition-all flex-shrink-0">
                  ...
                  <div className="p-6 pt-5 space-y-4 flex-grow">
                    ...
                    <div className="space-y-1">
                       ...
                       <div className="flex items-center justify-between">
                          ...
                          <div className={`px-2 py-0.5 rounded-lg text-[9px] font-black uppercase tracking-widest ${statusColor} shadow-sm`}>
                            {statusText}
                          </div>
                       </div>
                    </div>
                    ...
                    <div className="pt-2 space-y-2">
                      ...
                      <div className="w-full bg-slate-100/50 dark:bg-white/5 h-2 rounded-full overflow-hidden border border-white/5">
                        <div className="bg-gradient-to-r from-blue-500 to-indigo-600 h-full rounded-full transition-all duration-1000 shadow-[0_0_8px_rgba(99,102,241,0.5)]" style={{ width: `${enrollment.progress}%` }}></div>
                      </div>
                    </div>
                  </div>

                  <div className="px-6 pb-6 pt-2 grid grid-cols-2 gap-3 border-t border-slate-50 dark:border-white/5 mt-auto">
                    <button 
                      onClick={(e) => { e.stopPropagation(); onViewDetails(course); }}
                      className="flex items-center justify-center gap-2 py-2.5 glass dark:glass border border-slate-200 dark:border-white/10 text-slate-600 dark:text-slate-300 rounded-xl text-xs font-bold hover:bg-slate-100/50 dark:hover:bg-white/5 transition-colors shadow-sm"
                    >
                    ...
                    <button 
                      ...
                      className="flex items-center justify-center gap-2 py-2.5 bg-slate-900 dark:bg-blue-600 text-white rounded-xl text-xs font-bold hover:shadow-xl hover:shadow-blue-500/20 transition-all shadow-lg"
                    >
