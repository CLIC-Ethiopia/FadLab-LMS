...
import { Laptop, Github, Mail, Shield, Loader2, AlertCircle } from 'lucide-react';
      ...
      <div className="bg-white dark:bg-slate-900 p-8 rounded-2xl shadow-xl w-full max-w-md relative z-10 border border-slate-100 dark:border-slate-800 transition-colors">
        ...
        <div className="space-y-4">
          <button 
            onClick={() => onLogin()}
            disabled={isLoading}
            className="w-full flex items-center justify-center gap-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 p-3 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700 transition-all font-medium group shadow-sm disabled:opacity-70 disabled:cursor-not-allowed"
          >
            {isLoading ? <Loader2 className="w-5 h-5 animate-spin text-slate-500" /> : <img src="https://www.svgrepo.com/show/475656/google-color.svg" alt="Google" className="w-5 h-5" />}
          ...
          <button 
            onClick={() => onLogin()}
            disabled={isLoading}
            className="w-full flex items-center justify-center gap-3 bg-[#1877F2] text-white p-3 rounded-lg hover:bg-[#166fe5] transition-all font-medium shadow-sm disabled:opacity-70 disabled:cursor-not-allowed"
          >
             {isLoading ? <Loader2 className="w-5 h-5 animate-spin text-blue-200" /> : <svg className="w-5 h-5 fill-current" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>}
          ...
          <button 
            ...
            disabled={isLoading}
            className="w-full flex items-center justify-center gap-3 bg-slate-900 dark:bg-slate-800 text-white p-3 rounded-lg hover:bg-slate-800 dark:hover:bg-slate-700 border border-transparent dark:border-slate-700 transition-all font-medium shadow-sm group disabled:opacity-70 disabled:cursor-not-allowed"
          >
            {isLoading ? <Loader2 className="w-5 h-5 animate-spin text-slate-400" /> : <Shield className="w-5 h-5 text-slate-400 group-hover:text-white transition-colors" />}
